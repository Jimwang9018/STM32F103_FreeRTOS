...
static float         X[LENGTH]  = {0};              //  当前状态的预测值
...
//==                    卡尔曼滤波需要配置的变量                            ==//
//============================================================================//
static float         F[LENGTH]  = {1};              // 1 状态转移矩阵   即：坚信当前的状态与上一次状态的关系，如果坚信是一样的，则状态转移矩阵就为单位矩阵
extern float         Q[LENGTH]  = {0.05f};//0.0001f              //  系统过程的协方差	协方差的定义：真实值与期望值之差的平方的期望值
extern float         R[LENGTH]  = {4.0f};              //2  测量过程的协方差	协方差的定义：真实值与期望值之差的平方的期望值   
...
static float         Temp2[LENGTH] = {0};       //  辅助变量, 同时保存tCov.PPreOpt[]的初始化值
